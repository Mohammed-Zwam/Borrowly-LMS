...
export class ResetPasswordComponent implements OnInit {
  ...
  getPasswordErrors(): string[] {
    ...
    if (control && control.errors && (control.dirty || control.touched)) {
      ...
      if (control.errors['maxlength']) errors.push('Password cannot exceed 20 characters');
    }
    return errors;
  }
  ...
  getConfirmPasswordErrors(): string[] {
    ...
    if (formErrors && formErrors['passwordMismatch'] && (this.resetForm.get('confirmPassword')?.dirty || this.resetForm.get('confirmPassword')?.touched)) {
  ...
  hasPasswordMismatch(): boolean {
    return !!(this.resetForm.errors?.['passwordMismatch'] && this.resetForm.get('confirmPassword')?.touched);
  }
